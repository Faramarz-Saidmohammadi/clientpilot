...
import { getProjectById, listProjectFiles } from "@/actions/projects";
...
export default async function ProjectDetailPage({
  ...
  const files = (await listProjectFiles(id)) as unknown as RawFile[];
        ...
        <div className="mt-3 space-y-2 text-sm">
          {files.map((f) => (
            <a
              key={String(f._id)}
              href={f.url}
              className="block rounded-md border p-2 hover:bg-[color-mix(in_oklab,var(--background)_45%,transparent)]"
            >
      ...
      <Card>
        ...
        <div className="grid gap-2 md:grid-cols-3">
          {[
            ...
            <div key={tab} className="rounded-md border px-3 py-2 text-sm">
              {tab}
            </div>
          ))}
        </div>
