...
export default async function ProjectsPage({
  ...
  ])) as unknown as [RawProject[], RawClient[], { currency: string }];
      ...
      <div className="space-y-2">
        {projects.map((project) => (
          <Link
            ...
            href={`/${locale}/app/projects/${project._id}`}
            className="block rounded-md border p-3 hover:bg-[color-mix(in_oklab,var(--background)_45%,transparent)]"
          >
            ...
            <div className="text-sm text-[var(--muted)]">
              ...
              {t("budgetLabel")}:{" "}
              {currency(project.budget, settings.currency, safeLocale)}
            </div>
          </Link>
        ))}
      </div>
